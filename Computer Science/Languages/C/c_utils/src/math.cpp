...
float
noise_power_from_dataf (const _Complex float *data_noise,
                        const _Complex float *data_no_noise, size_t len)
{
  ASSERT (data_noise);
...
void
gen_awgn_from_snrdb (_Complex float *dest, size_t len, float sig_power,
                     float snrdb)
{
...
void
fftwf_dft_1d_one_time (size_t len, _Complex float *src,
                       _Complex float *dest, int dir, int type)
{
...
void
cf64_build_tuner_inl (_Complex float *dest, size_t len, float f_offset,
                      float fs, _Complex float c0)
{
...
_Complex float
cf64_tune_inl (_Complex float *dest, size_t len, float f_offset,
               float fs, _Complex float c0)
{
  float pinc = -2.0f * M_PI * f_offset / fs;
...
static void
linear_signal_ramp_up (_Complex float *sig, size_t k0, size_t len,
                       size_t rkf)
{
  ASSERT (sig);
  ...
  for (size_t k = k0; (k - k0) < len && k < rkf; ++k)
    {
      sig[k - k0] *= sqrtf (m * ((float)k - x0) + y0);
    }
}

static void
quadratic_signal_ramp_up (_Complex float *sig, size_t k0, size_t len,
                          size_t rkf)
{
  ASSERT (sig);
  ...
  // y = a(x^2) + bx + c
  float a = (y1 - y0) / ((x1 - x0) * (x1 - x0));

  for (size_t k = k0; (k - k0) < len && k < rkf; ++k)
    {
      float x = (float)k - x0;
      float power_scale = a * x * x + y0;
      sig[k - k0] *= sqrtf (power_scale);
    }
}

static void
hamming_signal_ramp_up (_Complex float *sig, size_t k0, size_t len,
                        size_t rkf)
{
  ASSERT (sig);
  ...
  float N = (float)(rkf - k0);
  for (size_t k = k0; (k - k0) < len && k < rkf; ++k)
    {
      float x = (float)(k - k0);
      float hamming_scale = 0.54f - 0.46f * cosf (2.0f * M_PI * x / (N - 1));
      sig[k - k0] *= sqrtf (hamming_scale);
    }
}
...
void
signal_ramp_up (_Complex float *sig, size_t k0, size_t len, size_t rkf,
                ramp_up type)
{
  switch (type)
    {
    case RU_LINEAR:
      ...
      break;
    case RU_QUADRATIC:
      quadratic_signal_ramp_up (sig, k0, len, rkf);
      break;
    case RU_HAMMING:
      hamming_signal_ramp_up (sig, k0, len, rkf);
    default:
      unreachable ();
    }
}
...
static void
linear_signal_ramp_down (_Complex float *sig, size_t k0, size_t len,
...
static void
quadratic_signal_ramp_down (_Complex float *sig, size_t k0,
                            size_t len, size_t rk0, size_t klen)
{
  ASSERT (sig);
  ...
  for (size_t k = MAX (rk0, k0); (k - k0) < len; ++k)
    {
      float x = (float)k - x0;
      float power_scale = a * x * x + y0;
      sig[k - k0] *= sqrtf (MAX (0.0f, power_scale));
    }
}

static void
hamming_signal_ramp_down (_Complex float *sig, size_t k0, size_t len,
                          size_t rk0, size_t klen)
{
  ASSERT (sig);
  ...
  float N = (float)(klen - rk0);
  for (size_t k = MAX (rk0, k0); (k - k0) < len && k < klen; ++k)
    {
      float x = (float)(k - rk0);
      float hamming_scale = 0.54f - 0.46f * cosf (2.0f * M_PI * x / (N - 1));
      sig[k - k0] *= sqrtf (hamming_scale);
...
void
signal_ramp_down (_Complex float *sig, size_t k0, size_t len, size_t rk0,
                  size_t klen, ramp_up type)
{
  switch (type)
    {
    case RU_LINEAR:
      ...
      break;
    case RU_QUADRATIC:
      quadratic_signal_ramp_down (sig, k0, len, rk0, klen);
      break;
    case RU_HAMMING:
      hamming_signal_ramp_down (sig, k0, len, rk0, klen);
      break;
    default:
      unreachable ();
    }
...
void
init_tuner_emat (_Complex float *dest, size_t match_len, frange f,
                 float fs)
{
...
siggen_sum_result_k
siggen_sum_sigs_k (_Complex float *dest, size_t dlen,
                   const pargs *ps, size_t pslen,
                   const krange k, const float fs)
{
...
TEST (siggen_sum_result_k)
{
  ...
  // Finite
  kres = siggen_sum_sigs_k (out, 200, ps, 4, (krange){ 0, 201, 0 }, 1);
  ...
  // Infinite
  kres = siggen_sum_sigs_k (out, 200, ps, 4, (krange){ 21, 0, 1 }, 1);
  TEST_EQUAL (kres.filled, 200lu, "%zu");
...
void
cf64_rot_row_had (_Complex float *dest, const _Complex float *left,
                  const _Complex float *right, size_t len, size_t rotnum)
{
  ASSERT (dest);
  ASSERT (left);
  ASSERT (right);
...
void
cf64_row_had (_Complex float *dest, const _Complex float *left,
              const _Complex float *right, size_t len)
{
  ASSERT (dest);
  ASSERT (left);
  ASSERT (right);
...
void
cf64_mat_vec_row_had (_Complex float *dest, const _Complex float *left_mat,
                      const _Complex float *right_vec, size_t rows,
                      size_t cols)
{
  ASSERT (dest);
  ASSERT (left_mat);
...
void
cf64_row_had_right_shift (_Complex float *dest, const _Complex float *left,
                          const _Complex float *right, size_t len,
                          long right_start)
{
  ASSERT (len <= LONG_MAX);
...
void
cf64_row_had_mult_vec (_Complex float *dest, const _Complex float *vec,
...
void
cf64_matmul_mat_vec_cf64abs (float *col_dest, const _Complex float *mat,
                             const _Complex float *col_vec, size_t rows,
                             size_t cols)
{
  ASSERT (col_dest);
  ASSERT (mat);
...
_Complex float
cf64_vec_inner_prod (const _Complex float *left,
                     const _Complex float *right, size_t len)
{
...
void
cf64_transpose_cf64_col_limit (_Complex float *dest,
                               const _Complex float *src, size_t rows,
...
void
cf64_transpose_f32_abs_sqrd_col_limit (float *dest,
...
void
zerof32_range (float *dest, intervals s)
{
  ASSERT (s.s1 >= s.s0);
  if (s.s1 > s.s0)
...
TEST (zerof32_range)
{
  ...
  float expected[] = { 1, 0, 0, 0, 5, 6 };
  intervals i = {
    .s0 = 1,
    .s1 = 4,
  };
  zerof32_range (data, i);
  TEST_EQUAL (memcmp (data, expected, 6 * sizeof (float)), 0, "%d");
}
...
TEST (zerocf_range)
{
  _Complex float data[] = { 1, 2, 3, 4, 5, 6 };
  _Complex float expected[] = { 1, 0, 0, 0, 5, 6 };
  intervals i = {
    .s0 = 1,
    .s1 = 4,
  };
  zerocf_range (data, i);
  TEST_EQUAL (memcmp (data, expected, 6 * sizeof (_Complex float)), 0, "%d");
}

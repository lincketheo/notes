...
TEST (frange_parse_argstr)
{
  ...
  // Invalid cases
  ret = frange_parse_argstr (&result,
                             "0.0:-0.1:1.0"); // df moves away from f1
  TEST_EQUAL (ret, -1, "%d");

  ret = frange_parse_argstr (&result,
                             "1.0:0.1:0.0"); // df moves away from f1
  TEST_EQUAL (ret, -1, "%d");
...
{
  return (size_t)floorf ((f.f1 - f.f0) / (f.df));
}
...
static si_prefixf prefixesf[] = {
  { "y", 1e-24 }, { "z", 1e-21 }, { "a", 1e-18 }, { "f", 1e-15 }, { "p", 1e-12 }, { "n", 1e-9 }, { "u", 1e-6 }, { "m", 1e-3 }, { "", 1.0 }, { "k", 1e3 }, { "M", 1e6 }, { "G", 1e9 }, { "T", 1e12 }, { "P", 1e15 }, { "E", 1e18 }, { "Z", 1e21 }, { "Y", 1e24 }
};
